...
# =========================
# 🚀 2단계: RSI + 볼린저밴드 파라미터 범위
# =========================
PARAM_RANGES_STAGE2 = {
    'rsi_period': [7, 9, 11, 14, 17, 20, 23, 26],  # 8개
    'rsi_oversold': [20, 25, 30, 35],  # 4개
    'rsi_overbought': [65, 70, 75, 80],  # 4개
...
def signal_engine_stage2(df: pd.DataFrame, **params):
    ...
    # 신호 생성 (1단계 + 2단계 조건)
    # LONG: EMA 크로스 + 가격변화 + RSI 과매도 + BB 하단 근처
    long_conditions = (
        (x["cr_prev"] <= 0) & (x["cr"] > 0) & 
    ...
    # SHORT: EMA 크로스 + 가격변화 + RSI 과매수 + BB 상단 근처
    short_conditions = (
        (x["cr_prev"] >= 0) & (x["cr"] < 0) & 
        (x["price_change"] < -price_change_threshold) &
...
def optimize_stage2():
    """🚀 2단계: RSI + 볼린저밴드 파라미터 최적화"""
    print("=== 🚀 2단계 최적화 시작: RSI + 볼린저밴드 ===")
    ...
    print(f"🚀 2단계 예상 실행 시간: 약 {len(valid_combinations) * 0.3 / 60:.1f}분")
    ...
    for i, (rsi_p, rsi_ovs, rsi_ovb, bb_p, bb_s) in enumerate(valid_combinations):
        ...
        if result['score'] > best_score:
            ...
            print(f"   파라미터: RSI={rsi_p}기간/{rsi_ovs}과매도/{rsi_ovb}과매수, BB={bb_p}기간/{bb_s}표준편차")
    ...
    for i, result in enumerate(top_20):
        ...
        print(f"     RSI: {params['rsi_period']:2d}기간/{params['rsi_oversold']:2d}과매도/{params['rsi_overbought']:2d}과매수, "
              f"BB: {params['bb_period']:2d}기간/{params['bb_std']:3.1f}표준편차")
        print()
    ...
    if best_params is not None:
        ...
        print(f"\n✅ 2단계 결과가 'stage2_rsi_bb_result.json'에 저장되었습니다")
        ...
        print(f"  RSI 기간 범위: {min(rsi_period_values)} ~ {max(rsi_period_values)}")
        print(f"  RSI 과매도 범위: {min(rsi_oversold_values)} ~ {max(rsi_oversold_values)}")
        print(f"  RSI 과매수 범위: {min(rsi_overbought_values)} ~ {max(rsi_overbought_values)}")
        print(f"  BB 기간 범위: {min(bb_period_values)} ~ {max(bb_period_values)}")
        print(f"  BB 표준편차 범위: {min(bb_std_values):.1f} ~ {max(bb_std_values):.1f}")
