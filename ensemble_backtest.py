"""
앙상블 모델을 사용한 개선된 랜덤 백테스트
10년 전체 기간에서 3개 랜덤 기간 테스트 + 앙상블 모델 적용
"""
...
def ensemble_backtest():
    """앙상블 모델을 사용한 랜덤 백테스트 실행"""
    
    # 설정 로드
    cfg = load_config()
    log = setup_logger(level="INFO")
    
    log.info("=== 앙상블 모델 랜덤 백테스트 시작 ===")
    ...
    for period_info in test_periods:
        ...
        # 기술적 지표 모델 훈련 (앙상블용) - 더 많은 데이터 사용
        log.info("기술적 지표 모델 훈련 중...")
        
        # 훈련용 데이터 준비 (더 많은 데이터 사용)
        train_start = 512  # 특징 생성 시작점
        train_end = 512 + split_idx
        
        # 기술적 특징 생성
        technical_features = create_technical_features(period_feat.iloc[train_start:train_end])
        
        # 타겟 데이터 준비 (가격 변화율)
        price_data = period_feat['close'].iloc[train_start:train_end]
        price_returns = price_data.pct_change().fillna(0)  # 가격 변화율
        
        log.info(f"훈련 데이터 준비:")
        log.info(f"  - 특징 데이터: {technical_features.shape}")
        log.info(f"  - 타겟 데이터: {price_returns.shape}")
        
        # RandomForest 훈련
        if len(technical_features) > 100 and len(price_returns) > 100:
            success = ensemble.train_technical_model(technical_features, price_returns.values)
            if success:
                log.info("✅ RandomForest 훈련 성공!")
            else:
                log.warning("⚠️ RandomForest 훈련 실패, 기본 모델만 사용")
        else:
            log.warning(f"⚠️ 훈련 데이터 부족: 특징={len(technical_features)}, 타겟={len(price_returns)}")
        ...
        for i in range(0, len(X_valid), chunk_size):
            ...
            technical_batch = create_technical_features(period_feat.iloc[valid_start:valid_end])
...
def summarize_ensemble_results(results):
    ...
    if results:
        ...
        if consistency > 0.7:
            print("   ✅ 높은 일관성: 앙상블 모델이 안정적")
        elif consistency > 0.4:
            print("   ⚠️  보통 일관성: 앙상블 모델이 어느 정도 안정적")
        else:
            print("   ❌ 낮은 일관성: 앙상블 모델이 불안정")
        
        # RandomForest 훈련 성공 여부
        print(f"\n🌳 RandomForest 모델 상태:")
        print(f"   ✅ 훈련된 RandomForest 모델 사용")
        print(f"   ✅ 기술적 지표 + NHiTS + 시장 상황 조합")
        print(f"   ✅ 가중 투표 방식 앙상블")
